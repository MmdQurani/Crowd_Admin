              ...
              <div className="flex flex-col w-full items-center" key={i}>
                ...
                <div
                  className={`w-2/3 mx-auto flex justify-between  ${pathname?.includes(item?.link) ? 'bg-gray-500 rounded  ' : ''
                    } flex flex-row items-center text-white  justify-between mb-6 mr-10 ml-[42px]`}>
                  <Link
                    to={item?.link}
                    className={`w-full p-3 bg-gray-500 rounded-md text-white  ${pathname?.includes(item?.link) ? 'w-full p-3 bg-gray-500 rounded-md text-white' : 'w-full p-3 bg-gray-500 rounded-md text-white'
                      } flex flex-row items-center justify-start`}>
                    <InlineSVG
                      src={item.icon}
                      className={`fill-white filter brightness-0 invert ${pathname?.includes(item?.link) ? 'fill-white filter brightness-0 invert ' : ' fill-white filter brightness-0 invert '
                        } ml-1 cursor-pointer`}
                    />
